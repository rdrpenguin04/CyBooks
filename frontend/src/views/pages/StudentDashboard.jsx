    ...
    useEffect(() => {
        if (allLessons && myCompletions)
            setUnfinishedLessons(
                allLessons.filter(
                    (x) => !myCompletions.find((y) => y.lesson === x.id)
                )
            );
                ...
                {myTodos ? (
                    myTodos.length > 0 ? (
                        myTodos.map((completion) => (
                            <div
                                className="block border rounded-xl min-w-80 max-w-80 min-h-80 max-h-80 m-2 hover:cursor-pointer"
                                onClick={() => {
                        ...
                        <p>
                            You're all caught up! To find a new lesson to start,
            ...
            <div className="flex flex-row overflow-auto">
                {unfinishedLessons ? (
                    unfinishedLessons.lesson > 0 ? (
                        ...
                        <div className="block">
                            <p>
                                Congratulations; you've completed every CyBooks
                                lesson! Now revel in your victory... or maybe
                                create your own lesson?
                            </p>
                            <p>
                                CyBooks instructor accounts are free; try
                                logging out and signing up for one!
                            </p>
                        </div>
                    )
                ) : (
